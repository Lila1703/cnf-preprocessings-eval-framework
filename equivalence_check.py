...
def _read_dimacs_clauses(file_path):
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            lits = [x for x in values if x != 0]
            has_clause_terminator = 0 in values
            if has_clause_terminator or lits:
                clauses.append(lits)
                for l in lits:
                    if abs(l) > max_var:
                        max_var = abs(l)
...
def build_diff_cnf(f_path, g_path, out_path, mode='F_and_not_G'):
    ...
    # If G has no clauses, then G is a tautology and ¬G is unsatisfiable.
    # Hence F ∧ ¬G is unsatisfiable regardless of F.
    if not G_clauses:
        out_clauses.append([])
    else:
